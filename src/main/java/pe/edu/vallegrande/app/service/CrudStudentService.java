...
public class CrudStudentService implements CrudServiceSpec<Student>, RowMapper<Student> {
	...
	private final String SQL_SELECT_BASE = "SELECT student_id, names, lastname, email, document_type, document_number, active FROM student";
	private final String SQL_INSERT = "INSERT INTO student (names, lastname, email, document_type, document_number) VALUES (?, ?, ?, ?, ?)";
	private final String SQL_UPDATE = "UPDATE student SET names=?, lastname=?, email=?, document_type=?, document_number=? WHERE student_id=?";
	...
	public List<Student> get(Student bean) {
		...
		try {
			...
			// La consulta
			sql = SQL_SELECT_BASE
					+ " WHERE names LIKE ? AND lastname LIKE ? AND email LIKE ? AND document_type LIKE ? AND document_number LIKE ?";
			pstm = cn.prepareStatement(sql);
	...
	public void update(Student bean) {
		...
		try {
			...
			pstm.setString(4, bean.getDocument_type());
			pstm.setString(5, bean.getDocument_number());
			pstm.setInt(6, bean.getStudent_id());
